...
export default async function GetCroppedImg(
  ...
  async function getImageFileSize(imageSrcData: any) {
    const response = await fetch(imageSrcData);
  ...
  const fileSize = await getImageFileSize(imageSrc);
  ...
  if (exp?.isExp == 1) {
    const croppedImageWidth = 500;
    const croppedImageHeight =
      (pixelCrop.height / pixelCrop.width) * croppedImageWidth;

    canvas.width = croppedImageWidth;
    canvas.height = croppedImageHeight;

    // ctx.drawImage(canvas, 0, 0, croppedImageWidth, croppedImageHeight)
